...
from chalicelib import __version__
from chalicelib.Engine.Factory import PreferredEngine
from chalicelib.CLI.Display import THREE_CHOICES
...
class OptionParser(optparse.OptionParser):
    ...
    def install_mqtt_options(self):
        ...
        #     action="store_true",
        #     dest="listen",
        #     help="listen and wait for mqtt command response by polling S3.\nWill not exit until response fetched or exits program",
        # )

        self.add_option_group(optgrp)

    def install_s3_options(self):
    ...
    def install_filecopy_options(self, help=None):
        """Install file copying specific options"""
        if help is not None:
            self.hide_titles.append("File copying:")
        ...
        optgrp.add_option(
            ...
            help=(
                help
                if help is not None
                else "copy local file or directory to remote nodes"
            ),
        )
        optgrp.add_option(
            "--rcopy",
            ...
            callback=self._copy_callback,
            help=(
                help if help is not None else "copy file or directory from remote nodes"
            ),
        )
        optgrp.add_option(
            "--dest",
            ...
            help=(
                help
                if help is not None
                else "destination file or directory on the nodes"
            ),
        )
        optgrp.add_option(
            "-p",
            action="store_true",
            dest="preserve_flag",
            help=help if help is not None else "preserve modification times and modes",
        )
        self.add_option_group(optgrp)

    def install_connector_options(self, help=None):
        ...
        optgrp.add_option(
            ...
            type="safestring",
            dest="user",
            help=help if help is not None else "execute remote command as user",
        )
        optgrp.add_option(
            "-o",
            "--options",
            action="store",
            dest="options",
            help=help if help is not None else "can be used to give ssh options",
        )
        optgrp.add_option(
            "-t",
            "--connect_timeout",
            ...
            help=help if help is not None else "limit time to connect to a node",
            type="float",
        )
        optgrp.add_option(
            "-u",
            ...
            help=(
                help
                if help is not None
                else "limit time for command to run on the node"
            ),
            type="float",
        )
        optgrp.add_option(
            ...
            help=(
                help
                if help is not None
                else "run mode; define MODEs in <confdir>/*.conf"
            ),
        )
        optgrp.add_option(
            "-R",
            ...
            help=(
                help
                if help is not None
                else "worker name to use for command execution "
        ...
        optgrp.add_option(
            ...
            help=(
                help
                if help is not None
                else "whether to enable remote execution: in tree "
    ...
    def format_help(self, formatter=None):
        ...
        # Filter out the help for a specific OptionGroup by its title
        return "\n".join(
            [
                line
                for line in result.split("\n")
                if line.strip() not in self.hide_titles
            ]
        )
