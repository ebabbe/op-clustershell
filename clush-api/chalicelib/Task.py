...
>>> from chalicelib.Task import task_self, NodeSet
>>>
...
from chalicelib.Defaults import config_paths, DEFAULTS
from chalicelib.Defaults import (
    _local_workerclass,
...
from chalicelib.Engine.Engine import EngineAbortException
from chalicelib.Engine.Engine import EngineTimeoutException
from chalicelib.Engine.Engine import EngineAlreadyRunningError
from chalicelib.Engine.Engine import EngineTimer
from chalicelib.Engine.Factory import PreferredEngine
from chalicelib.Worker.EngineClient import EnginePort, EngineClientError
from chalicelib.Worker.Popen import WorkerPopen
from chalicelib.Worker.Tree import TreeWorker
from chalicelib.Worker.Worker import FANOUT_UNLIMITED

from chalicelib.Event import EventHandler
from chalicelib.MsgTree import MsgTree
from chalicelib.NodeSet import NodeSet, expand

from chalicelib.Topology import TopologyParser, TopologyError
from chalicelib.Propagation import PropagationTreeRouter, PropagationChannel
...
class Task(object):
    ...
    def __init__(self, thread=None, defaults=None):
        ...
        if not getattr(self, "_engine", None):
            ...
            self._default.update(
                {
                    ...
                    "s3_worker": _s3_workerclass(defaults),
                }
            )
            ...
            self.msgtree_lock = threading.Lock()
    ...
    def schedule(self, worker):
        ...
        """
        assert (
            self in Task._tasks.values()
        ), "deleted task instance, call task_self() again!"

        # bind worker to task self
        worker._set_task(self)

        # add nodes to tasks nodes
        self.nodes.update(expand(worker.nodes))
    ...
    def _msgtree(self, sname, strict=True):
        ...
        if self.default("%s_msgtree" % sname):
            print(f"task id {id(self._msgtrees)}")
